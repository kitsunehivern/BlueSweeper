...
#define PASSWORD 2
    ...
    while (window.isOpen()) {
        ...
        // Sign up
        sf::Text signUpText("Sign up", font, 90);
        signUpText.setFillColor(sf::Color::Black);
        signUpText.setPosition((window.getSize().x - signUpText.getGlobalBounds().width) / 2, 90);
        ...
        if (valid && fs::exists("Data/Local/" + username)) {
            tmpMessage = "Username is not available, sensei";
            valid = false;
        }
        ...
        while (window.pollEvent(event)) {
            ...
            case sf::Event::MouseButtonReleased:
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    if (positionInBox(sf::Mouse::getPosition(window), confirmBox.getGlobalBounds())) {
                        if (valid) {
                            messageText.setString("Welcome, " + username + " sensei");
                            messageText.setFillColor(sf::Color::Black);
                            messageText.setPosition((window.getSize().x - messageText.getGlobalBounds().width) / 2, 2);
                            messageClock.restart();

                            fs::create_directory("Data/Local/" + username);
                            std::ofstream fout(("Data/Local/" + username + "/Password.txt").c_str());
                            fout << password;
                            fout.close();
